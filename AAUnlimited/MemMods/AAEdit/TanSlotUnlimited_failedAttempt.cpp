...
/*
 * Plan B: let him read tans into the Pa Slot, then copy them over and repeat
 */
...
namespace EditInjections {
	namespace TanSlotUnlimit {
		...
		/****************************************/
		int loc_currLine = TAN_MAX_LINE; //used to read the file in the first part of the injections
		int loc_currSlot = 0; //used for insertion in the second part of the injection
		...
		void __cdecl LoadLoopEndFillPa(BYTE* paEnd) {
			//note that paStructs have exactly (and hardcoded so) 8 members, each 0x28 apart, each being a pointer.
			//also, paEnd points behind the last member as it was increased
			for (int i = 7; i >= 0; i--) {
				paEnd -= 0x28;
				loc_paBuffer[loc_currLine - 14][i] = *(DWORD*)(paEnd);
				*(DWORD*)(paEnd) = General::GameBase + 0x36F650;
			}
		}
		...
		void LoadLoopEndInject() {
			...
			*/
			...
			DWORD address = General::GameBase + 0x116076;
		...
		/************************************/
		void _cdecl InsertLoopTestTest(BYTE* paStruct) {
			for (int i = 0; i < 8; i++) {
				*(DWORD*)(paStruct) = loc_paBuffer[loc_currSlot-5][i];
				paStruct += 0x28;
			}
		}

		void __declspec(naked) InsertLoopTestRedirect() {
			_asm {
				pushad
				lea eax, [esp+0x84]
				push eax
				call InsertLoopTestTest
				add esp, 4
				popad
				mov eax, [edx+0x13C] //original code
				ret
			}
		}
		...
		void InsertLoopTest() {
			...
			*/
			...
			/*DWORD address = General::GameBase + 0x1166DB;
			DWORD redirectAddress = (DWORD)(&InsertLoopTestRedirect);
			Hook((BYTE*)address,
			{ 0x8B, 0x82, 0x3C, 0x01, 0x00, 0x00 },
				{ 0xE8, HookControl::RELATIVE_DWORD, redirectAddress, 0x90 },
				NULL);*/
		} 
		...
		void InsertLoopCall() {
			...
			//note that edi gets destroyed due to our naive loop, so will have to set it to the correct value every time
		...
		void InsertLoopEndFindNext() {
			loc_currSlot++;
			while (loc_paBuffer[loc_currSlot-5][0] == General::GameBase + 0x36F650 && loc_currSlot <= TAN_MAX_SLOT) loc_currSlot++;
		}
