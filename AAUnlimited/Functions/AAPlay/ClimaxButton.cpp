...
namespace ClimaxButton {
	...
	void InitCfg() {
		...
		std::ifstream infile(General::AAUPath + L"resources\\climax_button_poses.txt");
		if (!infile) {
			LOGPRIO(Logger::Priority::WARN) << "[ClimaxBtn] Can't open the AAUnlimited/resources/climax_button_poses.txt file\r\n";
			return;
		}
		...
		// Binding a keys for 'Start Climax' (Num_1) and 'Start Normal Pose' (Num_3)
		GameTick::RegisterMsgFilter(GameTick::MsgFilterFunc([](MSG *m) {
			if (m->message != WM_KEYDOWN) 
				return false;
			if (m->wParam == VK_NUMPAD1)
				StartClimaxPose();
			else if (m->wParam == VK_NUMPAD3)
				StartNormalPose();
